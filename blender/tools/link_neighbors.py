...
RUN (on the district you want to edit; saves the .blend in place):
  blender --background assets/world_source/pieces/Piece_1_1.blend \
      --python blender/tools/link_neighbors.py -- [options]

Options (after --):
  --diagonals          also link the 4 diagonal neighbours (default: the 4 edge-adjacent ones)
  --all-districts      link EVERY built district's STREET (not just the 4/8 immediate
                       neighbours), each at its true world offset — for whole-world visual
                       context while hand-authoring one district's road/intersection content
                       (the case that matters most now that every district's roads move to the
                       hand-authored MANUAL workflow, not just border seams). Overrides
                       --diagonals (a superset). Heavier than the default (viewport cost only —
                       total built-world geometry is small, ~375k vertices across 36 districts,
                       so this is not a real performance concern, just more to look at).
...
def main():
    ...
    my_id = os.path.splitext(os.path.basename(blend))[0]
    piece = pr.piece_by_id(my_id)
    if piece is None or piece.get("grid") is None:
        print(f"ERROR: open a registered, grid-addressed piece .blend (pieces.json must have a "
              f"'grid' field for it) -- got: {os.path.basename(blend) or '(none)'}")
        sys.exit(2)
    gx, gy = piece["grid"]
    ...
    if "--all-districts" in argv:
        coords = [(nx, ny) for nx in range(wg.GRID_N) for ny in range(wg.GRID_N)
                  if (nx, ny) != (gx, gy)]
    else:
        offsets = [(-1, 0), (1, 0), (0, -1), (0, 1)]
        if "--diagonals" in argv:
            offsets += [(-1, -1), (-1, 1), (1, -1), (1, 1)]
        coords = [(gx + dx, gy + dy) for dx, dy in offsets
                  if 0 <= gx + dx < wg.GRID_N and 0 <= gy + dy < wg.GRID_N]

    linked = skipped = 0
    for nx, ny in coords:
        stem = wg.piece_id_for_cell(nx, ny)
        ...
        loc = ((nx - gx) * wg.DISTRICT, (ny - gy) * wg.DISTRICT, wg.elev_at(nx, ny) - my_elev)
