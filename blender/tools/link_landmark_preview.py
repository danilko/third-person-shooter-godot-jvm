...
Example (Dotonbori beside the harbor, gx=3 gy=0):
  blender --background world_master.blend --python tools/link_landmark_preview.py -- \\
      pieces/Piece_3_0.blend 3 0 Dotonbori

RUN all built districts (every non-void grid cell with an existing .blend on disk):
...
if argv[0] == "--all":
    ...
    for gy in range(wg.GRID_N):
        for gx in range(wg.GRID_N):
            ...
            ok = _link_one(abspath, gx, gy, stem, preview)
            linked += ok
            skipped += not ok
