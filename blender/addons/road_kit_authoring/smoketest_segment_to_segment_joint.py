...
RUN: blender --background --python addons/road_kit_authoring/smoketest_segment_to_segment_joint.py
"""
import bpy
import math
import os
...
def _tangent_deg(spine, end):
    pts = spine.data.splines[0].points
...
def main():
    ...
    n_pts_before = len(spine_b.data.splines[0].points)
    ...
    far_b_before = tuple(spine_b.data.splines[0].points[-1].co)[:3]
    ...
    spine_b = seg_b.objects[seg_b["rka_curve_object"]]
    port_b_a = next(o for o in seg_a.objects if o.get("rka_port") == "B")
    pts_b = spine_b.data.splines[0].points
