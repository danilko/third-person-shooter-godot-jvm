...
class RKA_SceneSettings(bpy.types.PropertyGroup):
    ...
    group_include_west: bpy.props.BoolProperty(
        name="West", default=False,
        description="Include the west neighbour in 'Open Group' (see ops_group_edit.py)")
    group_include_east: bpy.props.BoolProperty(
        name="East", default=False,
        description="Include the east neighbour in 'Open Group'")
    group_include_south: bpy.props.BoolProperty(
        name="South", default=False,
        description="Include the south neighbour in 'Open Group'")
    group_include_north: bpy.props.BoolProperty(
        name="North", default=False,
        description="Include the north neighbour in 'Open Group'")
    group_extra_stems: bpy.props.StringProperty(
        name="Other Pieces", default="",
        description="Comma-separated piece ids (e.g. 'Piece_3_4, Piece_0_2') not adjacent to "
                     "this one, AND/OR freestanding piece ids (e.g. 'Piece_2_3_b', the bridge) "
                     "-- for reshaping a freestanding piece's touchdown ramp against the piece "
                     "it lands on. Included when opening a group, or pulled into an already-open "
                     "group session with 'Add District(s)'")
