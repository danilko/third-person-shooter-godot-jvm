...
RUN: blender --background --python addons/road_kit_authoring/smoketest_dual_end_link.py
"""
import bpy
import math
import os
...
def main():
    ...
    p0 = Vector(spine.data.splines[0].points[0].co[:3])
    p1 = Vector(spine.data.splines[0].points[1].co[:3])
    p2 = Vector(spine.data.splines[0].points[2].co[:3])
    ...
    p_start = Vector(spine.data.splines[0].points[0].co[:3])
    p_end = Vector(spine.data.splines[0].points[-1].co[:3])
    ...
    seg_coll = opint.local_collection(seg_coll.name)
    spine = seg_coll.objects[seg_coll["rka_curve_object"]]
    p_start_after = Vector(spine.data.splines[0].points[0].co[:3])
    p_end_after = Vector(spine.data.splines[0].points[-1].co[:3])
