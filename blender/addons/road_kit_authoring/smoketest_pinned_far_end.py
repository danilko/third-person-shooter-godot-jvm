...
RUN: blender --background --python addons/road_kit_authoring/smoketest_pinned_far_end.py
"""
import bpy
import math
import os
...
def main():
    ...
    _assert(len(seg_spine.data.splines[0].points) == 2,
            "sanity: a freshly-extended segment should be a plain 2-point straight line, got %d"
            % len(seg_spine.data.splines[0].points))

    far_before = tuple(seg_spine.data.splines[0].points[-1].co)[:3]
    lane_before = seg_spine.data.splines[0].points[-1].radius
    ...
    seg_spine = bpy.data.objects.get(seg_coll.get("rka_curve_object"))
    pts = seg_spine.data.splines[0].points
    _assert(len(pts) == 3,
