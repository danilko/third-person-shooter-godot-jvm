...
def _nearby_outward_direction(pos, endpoints, radius=ARM_LINK_RADIUS):
    ...
    for p0, p1, tangent in endpoints:
        for near, sign in ((p0, 1.0), (p1, -1.0)):
            d = math.hypot(near[0] - pos[0], near[1] - pos[1])
            if d < best_dist:
                best_dist = d
                best_dir = (tangent[0] * sign, tangent[1] * sign)
    return best_dir
...
def _gather(context):
    ...
    for coll in bpy.data.collections:
        ...
        if spine_obj is None or spine_obj.type != 'CURVE' or spine_obj.name not in scene.objects:
            continue
        pts = ops_segment._spine_control_points(spine_obj)
        if len(pts) < 2:
            continue
        p0, p1 = pts[0], pts[-1]
        tangent = k.vnorm(k.vsub((p1[0], p1[1]), (p0[0], p0[1])))
        endpoints.append(((p0[0], p0[1]), (p1[0], p1[1]), tangent))
    ...
    for obj in scene.objects:
        ...
        if "rka_arm_name" in keys:
            ...
            tangent = k.vnorm(k.vsub((pb.x, pb.y), (pa.x, pa.y)))
