"""graph_nodes.py -- the Geometry Nodes vocabulary for the mesh-graph road build.

FOUR GROUPS, AND EVERY ROAD BAND IS ONE OF THEM. Curb, median, footway, carriageway and deck
...
finish bracket the stack.
    ...
    GN_GraphSpine    mesh carrier -> curve, and store the lateral frame `rka_lat` ONCE
    GN_GraphBand     sweep a unit line scaled per point by a named width attribute
    GN_GraphDeck     the same band, extruded DOWN by a per-point thickness attribute
    GN_GraphAssets   tile a palette asset along the curve, picked per point by an int attribute
    GN_GraphFinish   drop the carrier curve, keep the meshes

WHY PER-POINT WIDTH IS `Curve to Mesh`'s `Scale` FIELD AND NOT THE CURVE RADIUS. This Blender no
longer scales a swept profile by the curve's `radius` implicitly -- a Set Curve Radius of 5.25
still sweeps a unit-wide ribbon. `Scale` is a genuine per-point field and is the only thing that
makes a road whose width changes along its length expressible at all.

WHY THE WRAPPER OWNS BOTH OFFSETS. `_offset_stage` shifts the carrier curve sideways along
`rka_lat` and vertically before handing it to an inner group, and it accepts BOTH a constant and a
named per-point attribute for each. The per-point vertical offset is what lets a footway sit on
top of a kerb whose height varies per edge -- a constant-only Z would force one kerb height for
the whole network.

NONE OF THESE GROUPS KNOWS WHAT A LANE IS. Every number they read was computed by
`graph_solve.derived_offsets` from `lane_profile`. That is the same discipline
`road_stack.py` records: if a width is wrong it is wrong in one Python function, not somewhere in
a graph.
"""
import bpy
...
def make_spine_group():
    """GN_GraphSpine -- mesh carrier to curve, plus the lateral frame every layer shares.
    ...
    ng, existed = _new_group("GN_GraphSpine")
    if existed:
        return ng
    ifc = ng.interface
...
def make_finish_group():
    ...
    proxy pass."""
    ng, existed = _new_group("GN_GraphFinish")
    if existed:
        return ng
    ifc = ng.interface
    ifc.new_socket("Geometry", in_out="INPUT", socket_type="NodeSocketGeometry")
...
def make_band_group():
    """GN_GraphBand -- a flat ribbon of per-point width. The carriageway, the median, and both
    footways are all this group with a different width attribute and offset."""
    ng, existed = _new_group("GN_GraphBand")
    if existed:
        return ng
    ifc = ng.interface
    ifc.new_socket("Geometry", in_out="INPUT", socket_type="NodeSocketGeometry")
...
def make_deck_group():
    """GN_GraphDeck -- the structure under the carriageway: the same band, extruded DOWN by a
    per-point thickness.
    ...
    ng, existed = _new_group("GN_GraphDeck")
    if existed:
        return ng
    ifc = ng.interface
    ifc.new_socket("Geometry", in_out="INPUT", socket_type="NodeSocketGeometry")
...
def make_assets_group():
    """GN_GraphAssets -- tile a palette asset along the curve, picked PER POINT by an int
    attribute. Kerb pieces, pillars, railings, streetlights and props are all this group.

    THE TWO CONTRACTS THIS DEPENDS ON, both measured (see `graph_assets`'s docstring and
    `smoketest_graph_solve`), both silent-wrong-answer if broken:
      * `Collection Info (Separate Children)` emits children in link order, so `graph_assets`
    ...
    ng, existed = _new_group("GN_GraphAssets")
    if existed:
        return ng
    ifc = ng.interface
    ifc.new_socket("Geometry", in_out="INPUT", socket_type="NodeSocketGeometry")
...
def make_pillars_group():
    """GN_GraphPillars -- support columns from the deck soffit DOWN TO THE GROUND, at spacing.
    ...
    The height itself is resolved in Python (`graph_build.build_carrier` writes `rka_pillar_h` from
    ...
    ng, existed = _new_group("GN_GraphPillars")
    if existed:
        return ng
    ifc = ng.interface
    ifc.new_socket("Geometry", in_out="INPUT", socket_type="NodeSocketGeometry")
