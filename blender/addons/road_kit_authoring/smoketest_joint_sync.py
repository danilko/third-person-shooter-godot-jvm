...
RUN: blender --background --python addons/road_kit_authoring/smoketest_joint_sync.py
"""
import bpy
import math
import os
...
def main():
    ...
    p0_before = tuple(seg1_spine.data.splines[0].points[0].co)[:3]
    p1_before = tuple(seg1_spine.data.splines[0].points[1].co)[:3]
    ...
    seg1_spine = bpy.data.objects.get(seg1_coll.get("rka_curve_object"))
    p0_after = tuple(seg1_spine.data.splines[0].points[0].co)[:3]
    p1_after = tuple(seg1_spine.data.splines[0].points[1].co)[:3]
    ...
    joint_pt = next(p for p in seg1_spine.data.splines[0].points
    ...
    seg1_spine = bpy.data.objects.get(seg1_coll.get("rka_curve_object"))
    last_pt = seg1_spine.data.splines[0].points[-1]
