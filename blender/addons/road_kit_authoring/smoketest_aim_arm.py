...
RUN: blender --background --python addons/road_kit_authoring/smoketest_aim_arm.py
"""
import bpy
import math
import os
...
def main():
    ...
    p0 = tuple(spine.data.splines[0].points[0].co)[:3]
    p1 = tuple(spine.data.splines[0].points[1].co)[:3]
    ...
    p0new = tuple(new_spine.data.splines[0].points[0].co)[:3]
