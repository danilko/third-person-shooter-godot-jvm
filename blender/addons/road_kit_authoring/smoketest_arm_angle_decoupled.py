...
RUN: blender --background --python addons/road_kit_authoring/smoketest_arm_angle_decoupled.py
"""
import bpy
import math
import os
...
def main():
    ...
    far_before = tuple(spine.data.splines[0].points[-1].co)[:3]
    ...
    spine = seg_coll.objects[seg_coll["rka_curve_object"]]
    p_start_after = tuple(spine.data.splines[0].points[0].co)[:3]
    far_after = tuple(spine.data.splines[0].points[-1].co)[:3]
