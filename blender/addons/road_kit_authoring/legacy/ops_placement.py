...
KIT_LIBRARY_HOLDER_COLLECTION = "RKA_KitLibraryHolder"


def _kit_library_holder_collection(context):
    """A dedicated, hidden collection that holds a REAL (non-orphanable) local reference to every
    linked kit piece Collection -- 2026-08, root-caused a real bug (confirmed against
    `world_session.blend`): a piece is resolved everywhere else purely by NAME
    (`ops_intersection._resolve_curb_asset` does a `bpy.data.collections.get(name)` lookup off the
    `rka_*_asset_collection` custom property string), never via a real ID reference to the linked
    Collection itself. A linked Collection with no real referencer anywhere in the local file is
    dropped on save (Blender's normal "orphan data" behavior) -- even though an OBJECT pulled out
    of it and wired into a GN modifier's Instance input stays alive (that's a real reference), the
    enclosing Collection is not, so the very next fresh build/rebuild that needs to resolve that
    same name by string finds nothing: this was the concrete cause of "the lamp/asset piece
    silently stops working, even right after clicking Link/Set" -- the library was linked, then
    silently dropped again on the next save, with no error anywhere since `_resolve_curb_asset`'s
    contract is exactly "return None for an unresolvable name, caller skips quietly."

    Parenting every linked piece here the moment `RKA_OT_link_kit_library`/`RKA_OT_
    link_curb_kit_library` links it gives it exactly that real referencer, so it survives every
    save/reload from then on. Hidden (`hide_viewport`/`hide_render`) so it doesn't clutter the
    Outliner/viewport -- purely a keep-alive anchor, never meant to be seen or edited directly."""
    coll = bpy.data.collections.get(KIT_LIBRARY_HOLDER_COLLECTION)
    if coll is None:
        coll = bpy.data.collections.new(KIT_LIBRARY_HOLDER_COLLECTION)
...
class RKA_OT_link_kit_library(bpy.types.Operator):
    ...
    def execute(self, context):
        if not os.path.exists(paths.KIT_BLEND):
            self.report({'ERROR'}, "Kit library not found: %s (run tools/build_lane_kit.py first)" % paths.KIT_BLEND)
            return {'CANCELLED'}
        ...
        if not linked:
            ...
            return {'CANCELLED'}
        # See _kit_library_holder_collection's own docstring -- without this, a linked piece with
        # no OTHER real referencer in the file gets silently dropped on the next save.
        holder = _kit_library_holder_collection(context)
        for c in linked:
...
class RKA_OT_link_curb_kit_library(bpy.types.Operator):
    ...
    def execute(self, context):
        if not os.path.exists(paths.CURB_KIT_BLEND):
            self.report({'ERROR'}, "Curb kit not found: %s (run tools/build_curb_kit.py first)"
        ...
        if not linked:
            ...
            return {'CANCELLED'}
        # See _kit_library_holder_collection's own docstring -- without this, a linked piece (e.g.
        # a median/sidewalk/traffic-light asset referenced only by NAME off a custom property) has
        # no real referencer anywhere in the file and gets silently dropped on the next save --
        # the confirmed root cause of "the lamp/asset piece stops working even right after
        # clicking Link/Set."
        holder = _kit_library_holder_collection(context)
        for c in linked:
            if c.name not in holder.children:
