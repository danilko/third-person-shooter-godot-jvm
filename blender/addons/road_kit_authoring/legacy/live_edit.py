...
def rebuilding():
    ...
    `RKA_OT_adjust_segment_lanes` rewriting `spine_io.points(spine_obj)[i].radius` before
...
def _translate_spine(spine_obj, delta):
    ...
    if not spine_io.has_points(spine_obj):
        return
    for p in spine_io.points(spine_obj):
...
def _segment_joint_state(target_obj):
    ...
    # `is_spine`, not `type == 'CURVE'`: a modifier-stack piece carries its spine on a MESH, and
    # the tangent maths below reads it through `spine_io.points` either way. Left as a type check,
    # a segment joined to a stack piece silently got no tangent match at all -- the joint would
    # carry position and nothing else, which is the exact bug this function was written to fix.
...
def _ensure_bend_room(spine_obj, this_end, bend_fraction=0.15, max_bend_len=10.0):
    ...
    # Grow by one, then rewrite all three -- Blender's curve API appends only, never inserts, so
    # an insertion is always "append, then shift the values along". `spine_io.append_point` does
    # the same for a mesh carrier (adding the vertex AND the edge that chains it).
    pts = spine_io.append_point(spine_obj)
...
def move_dependent_marker(coll, obj, target_obj):
    ...
    # Carrier-agnostic -- see the same note in `_segment_joint_state`. Everything below goes
    # through `spine_io`, so a MESH-carried spine bends, grades and syncs identically.
    if not spine_io.is_spine(spine_obj):
        return
...
def _flush_rebuilds():
    ...
    with rebuilding():
        ...
        if markers:
            _break_stale_links()
        # Lane ports last: they are derived from the piece's exported lanes, so they must be
        # re-read AFTER every rebuild in this flush has run, not per-piece as each one finishes
        # (a cascade can rebuild the same piece twice). No-op for pieces that never opted in.
        from . import ops_lane_ports
        ops_lane_ports.refresh_if_present(ctx, inter | seg | curve_seg | curve_transition)
