...
RUN: blender --background --python-exit-code 1 --python addons/road_kit_authoring/smoketest_ports.py
"""
import bpy
import os
import sys
...
def main():
    ...
    p0 = spine_io.points(new_spine)[0].co
    p1 = spine_io.points(new_spine)[-1].co
    ...
    spine_io.points(orig_spine)[-1].co = (30.0, 0.0, 0.0, 1.0)   # was (20,0,0)
    ...
    orig_spine = bpy.data.objects.get(coll["rka_curve_object"])
    spine_io.points(orig_spine)[-1].co = (50.0, 0.0, 0.0, 1.0)
