...
tail, no manual button). A PROFILE-style median chain of 2+ linked segments should collapse into ONE
continuous merged row object (`curb_medianchain_<n>` in the dedicated `RKA_MedianChains`
collection), with each member's own individual `curb_<coll>_median` object removed -- otherwise the
two would visibly overlap. (2026-08: median collapsed to a NONE/PROFILE-only system -- see
`ops_intersection.MEDIAN_STYLE_ITEMS` -- `median_merge.py` still merges via `curb_asset_row`'s
discrete tiled row internally -- the old 'ASSET' style's own mechanism, kept as a library primitive
specifically for this chain-continuity use case -- only the STYLE NAME that gates it changed, from
'ASSET' to 'PROFILE'; see `median_merge.py`'s own docstring.)
...
RUN: blender --background --python-exit-code 1 --python addons/road_kit_authoring/smoketest_median_chain_merge.py
"""
import bpy
import math
import os
...
def main():
    ...
    # --- Segment_1: off intersection A, port_A near the arm, port_B far -- a real median (PROFILE,
    # 2026-08: the only mergeable style now -- see median_merge.py's own docstring).
    ...
    ret = bpy.ops.rka.extend_from_arm(
        'EXEC_DEFAULT', arm_name="N", length=40.0, median_width=4.0, median_style='PROFILE',
    ...
    _assert(seg1.get("rka_median_style") == "PROFILE", "sanity: median style should be PROFILE")
    n1_before = len(spine_io.points(bpy.data.objects[seg1["rka_curve_object"]]))

    # --- Segment_2: off a SEPARATE intersection B, far away -- also a real PROFILE median.
    ...
    ret = bpy.ops.rka.extend_from_arm(
        'EXEC_DEFAULT', arm_name="S", length=40.0, median_width=4.0, median_style='PROFILE',
        median_asset_collection='Kit_Median_YellowSeparator')
    ...
    n2_before = len(spine_io.points(bpy.data.objects[seg2["rka_curve_object"]]))
    ...
    n1_after = len(spine_io.points(bpy.data.objects[seg1["rka_curve_object"]]))
    n2_after = len(spine_io.points(bpy.data.objects[seg2["rka_curve_object"]]))
    ...
    # --- ONE merged row object (not one per side -- there IS no "side" any more, see
    # median_merge.py's own docstring: the median's own centerline is exactly the spine) spanning
    # the full chain.
    ...
    _assert(len(merged) == 1, "expected exactly ONE merged row object (a single centerline, not "
            "the old per-side pair), got %d: %s" % (len(merged), [o.name for o in merged]))
    obj = merged[0]
    # A merged PROFILE/ASSET-row object is a Mesh point-cloud sampled at `median_asset_spacing` intervals
    # along the WHOLE concatenated centerline (kit_common.curb_asset_row -> sample_polyline) --
    # unlike the old curb_loop-based BOX wall, its point count has nothing to do with the spine's
    # own control-point count, and its points live in obj.data.vertices, not a Curve's splines.
    n_pts = len(obj.data.vertices)
    _assert(n_pts > 0, "%s should have at least some sampled instance points, got 0" % obj.name)
    print("median_chain_merge smoketest: exactly one merged row object exists (%d sampled "
          "instances across the full chain)" % n_pts)

    # --- both segments sit exactly on the X axis (seg1: (12,0)->(52,0); seg2 dual-pinned by the
    # rigid translate to also land on the X axis) -- a CORRECTLY ordered/reversed concatenation of
    # the spine centerline must therefore be perfectly MONOTONIC in X (12 -> 52 -> ... -> 92,
    # never doubling back). The natural ~6m spacing around the bend point is expected and NOT
    # itself a problem -- a naive small "gap at the seam" threshold doesn't distinguish that from
    # a real mis-ordering, this monotonicity check does.
    pts = [tuple(v.co)[:3] for v in obj.data.vertices]
    xs = [p[0] for p in pts]
    _assert(all(xs[i] < xs[i + 1] for i in range(len(xs) - 1)),
            "%s: X coordinates should be strictly increasing (12 -> 52 -> ... -> 92) for a "
            "correctly ordered concatenation of two collinear segments -- got %s" % (obj.name, xs))
    # No gap between consecutive sampled instances should exceed ~1.5x the requested spacing -- a
    # bigger gap would mean the row broke (restarted sampling) at the joint instead of the whole
    # chain being sampled as ONE continuous polyline, which is the entire point of this feature.
    spacing = seg1.get("rka_median_asset_spacing", 3.0)
    gaps = [math.dist(pts[i][:2], pts[i + 1][:2]) for i in range(len(pts) - 1)]
    _assert(max(gaps) < spacing * 1.5,
            "%s: no gap between consecutive median instances should exceed ~1.5x the %.1fm "
            "spacing -- a bigger gap means sampling restarted at the joint, got max gap %.2fm"
            % (obj.name, spacing, max(gaps)))
    print("median_chain_merge smoketest: the seam between the two members is continuous "
          "(no mis-ordered/mis-reversed jump, no restarted sampling gap at the joint)")

    # --- endpoints land exactly where expected: the merged row's FIRST point is seg1's own far/
    ...
    # order "last" point is its own natural FIRST point/port_A near arm S). Unlike the old
    # side-offset wall, the merged row sits EXACTLY on the spine centerline now, so this is a
    # tight tolerance, not the old "~2-3m median offset" one.
    seg1_spine = bpy.data.objects[seg1["rka_curve_object"]]
    seg2_spine = bpy.data.objects[seg2["rka_curve_object"]]
    seg1_port_a = tuple(spine_io.points(seg1_spine)[0].co)[:3]
    seg2_port_a = tuple(spine_io.points(seg2_spine)[0].co)[:3]
    d_first = math.dist(pts[0][:2], seg1_port_a[:2])
    d_last = math.dist(pts[-1][:2], seg2_port_a[:2])
    _assert(d_first < 0.5, "%s: first point should be exactly at seg1's port_A (near arm N), off "
                            "by %.2fm" % (obj.name, d_first))
    _assert(d_last < 0.5, "%s: last point should be exactly at seg2's port_A (near arm S), off by "
                           "%.2fm" % (obj.name, d_last))
    print("median_chain_merge smoketest: merged row endpoints land at each chain terminus' own "
          "unconnected end, confirming seg2 was walked in the correct (reversed) direction")

    # --- each member's own individual median object is gone (superseded by the merge).
    for coll in (seg1, seg2):
        name = "curb_%s_median" % coll.name
        _assert(coll.objects.get(name) is None,
                "%s's own individual median object '%s' should have been removed once merged"
                % (coll.name, name))
    print("median_chain_merge smoketest: each member's own individual median object was removed")
