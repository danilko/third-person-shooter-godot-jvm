...
RUN: blender --background --python-exit-code 1 --python addons/road_kit_authoring/smoketest_set_arm_angle.py
"""
import bpy
import math
import os
...
def main():
    ...
    p0_before = tuple(spine_io.points(seg_spine)[0].co)[:3]
    p1_before = tuple(spine_io.points(seg_spine)[1].co)[:3]
    ...
    seg_spine = bpy.data.objects.get(seg_coll.get("rka_curve_object"))
    p0_after = tuple(spine_io.points(seg_spine)[0].co)[:3]
    p1_after = tuple(spine_io.points(seg_spine)[1].co)[:3]
