...
session building a 2-district streaming test: an unshifted check reported garbage). For a
`District_<theme>_<gx>_<gy>`-named sidecar, `--offset-a`/`--offset-b` default to that district's
own `lib/world_grid.py` `district_center`/`elev_at` placement automatically; anything else
(a hand-named test district, an overlay) needs an explicit `x,y,z` on the command line.
...
_DISTRICT_RE = re.compile(r"^District_[A-Za-z]+_(\d+)_(\d+)$")
...
def _auto_offset(stem):
    """(x, y, z) world placement for a `District_<theme>_<gx>_<gy>` stem, via the SAME
    `lib/world_grid.py` numbers `towns/build_world.py`'s region markers/`WorldZoneMarker`
    placement actually use -- or None if the stem doesn't match that naming (a hand-named test
    district, an overlay: caller must pass an explicit --offset-* or accept (0,0,0))."""
    m = _DISTRICT_RE.match(stem)
    if not m:
        return None
    gx, gy = int(m.group(1)), int(m.group(2))
    import world_grid as wg
    if not (0 <= gx < wg.GRID_N and 0 <= gy < wg.GRID_N):
        return None   # off-grid test district (e.g. District_test_8_8) -- no auto placement
    cx, cy = wg.district_center(gx, gy)
    return (cx, cy, wg.elev_at(gx, gy))
...
def main():
    ...
    if not cross:
        print("WARNING: no boundary connection found between '%s' and '%s' at these offsets -- "
              ...
              "--offset-a/--offset-b explicitly for anything that isn't a District_<theme>_<gx>_"
              "<gy>-named district on the real world grid)." % (stem_a, stem_b, tolerance))
