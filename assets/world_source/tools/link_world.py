...
# (towns/build_world.py) — the overview links every district directly below, so pulling the
...
# link/instance mechanics live in kit_common (shared with towns/build_world.py, whose LAYOUT
...
def main():
    ...
    for gy in range(wg.GRID_N):
        for gx in range(wg.GRID_N):
            stem = wg.piece_stem(gx, gy, wg.theme_at(gx, gy))
    ...
    # ── overlays (separate always-resident files, world-positioned — §5 convention) ──
    n_overlay = 0
    overlay_dir = os.path.join(ROOT, "overlays")
    if os.path.isdir(overlay_dir):
        for fn in sorted(os.listdir(overlay_dir)):
            if not (fn.startswith("Overlay_") and fn.endswith(".blend")):
                continue
            for c in _link_collections(os.path.join(overlay_dir, fn), ["OVERLAY", "MANUAL"]):
                _instance(dest, f"{fn[:-len('.blend')]}_{c.name}", c, (0.0, 0.0, 0.0))
                n_overlay += 1
            print(f"  linked overlay {fn}")
    ...
    print(f"link_world: saved {out_path} — {linked} districts linked, {missing} missing, "
          f"{n_master} master collections, {n_overlay} overlay collections")
