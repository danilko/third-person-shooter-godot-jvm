...
assets/world_source/kits/
  <kit_id>/                    one folder per kit = one licence and one module grid
    ...
    <kit_id>.blend             every piece on a grid, one collection per piece (blender/tools/build_building_kit_blend.py)
    source/                    the download's glTF, untouched (.gdignore: Godot never imports it)
    textures/                  the download's images, shared by every piece (and by the .blend, not packed)
    pieces/<category>/         GENERATED by normalize_kit.py: module_scale baked into the vertices
    pieces.json                GENERATED: category and measured size of every piece
...
(copy `quaternius_downtown_city/kit.json`, measure its module and storey, write its category rules and rows),
then name it as a type's `kit`. Credit it in `CREDITS.md`. Map data is CC0/MIT only. A download is never split
...
- **PLACEHOLDER status:** the pieces are still generated from `source/`, so an edit in a kit `.blend` is not
  exported yet and a rebuild overwrites it. PLAN.md 3.6b step 1 makes the kit `.blend` the owner, and step 2
  decides who owns a material's look. The Blender files show `MI_Trim_MetalConcrete` dark because its grey
