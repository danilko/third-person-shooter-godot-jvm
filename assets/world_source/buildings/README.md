...
## The nature kit (`kits/quaternius_stylized_nature/`, CC0)

68 vegetation and rock models -- trees, bushes, ferns, grasses, clover, flowers, mushrooms, petals, boulders,
cobbles and stepping stones. It is a kit of UNRELATED PROPS, so it has no plan module and no storey, and
`kit.json` carries `category_scale` instead of one `module_scale`: **one scale per family, baked into the
vertices** (the kit rule -- nothing is scaled at placement). Every number in it was measured; the record is
`kits/quaternius_stylized_nature/STUDY.md`, regenerated by

    python3 tools/building_kit/study_nature_kit.py --md assets/world_source/kits/quaternius_stylized_nature/STUDY.md
    godot --path . --script tools/godot/shot_nature_kit.gd -- <dir> [--variants] [--before]   # needs a display

The trees came out the right size and are baked at 1.0; everything at ground level came out 2.4-10x too large
(a clover leaf stood 1.26 m) and is baked down. The pieces are placed like any other kit's, by name --
`quaternius_stylized_nature:CommonTree_3` in a type's `props` -- and their materials resolve to
`<kit>/materials/MI_*.tres` the usual way. Placing them in the WORLD (street trees from the road facts, forest
scatter per region) is PLAN.md 3.16 step 4 and is not built.

**A recolour is one material, not a new texture.** Each leaf shape ships twice: `X.png` is a pure white mask and
`X_C.png` is the same shape already tinted, so sakura, a fresh-green keyaki, an autumn ginkgo and a momiji are
one `albedo_color` each over the white one. `shot_nature_kit.gd --variants` renders the five candidates.
