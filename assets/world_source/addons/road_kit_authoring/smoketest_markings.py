...
RUN: blender --background --python addons/road_kit_authoring/smoketest_markings.py
"""
import bmesh
import bpy
import os
...
def main():
    ...
    _assert(len(marks) == 2, "expected 2 mark_* objects (1 yellow + 1 white), got %d" % len(marks))
    yellow = next(o for o in marks if "_yellow_" in o.name)
    white = next(o for o in marks if "_white_" in o.name)
    ...
    _assert(len(marks_after) == 2, "expected 2 mark_* objects after gap rebuild, got %d (leak/dup?)"
            % len(marks_after))
    white_after = next(o for o in marks_after if "_white_" in o.name)
    ...
    _assert(len(marks_twice) == 2, "expected still 2 mark_* objects after a second rebuild, got %d "
                                    "(accumulation?)" % len(marks_twice))
    print("markings smoketest: gap [0.4, 0.6] persisted across rebuild, no geometry in gap, "
          "no duplicate accumulation across repeated rebuilds")
