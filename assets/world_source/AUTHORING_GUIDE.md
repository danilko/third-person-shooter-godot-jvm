...
Conventions when the first overlay is built:

- Authored in **world coordinates** (like the master's own content), exported with the same
  `tools/export_world.py` + WorldBaker path to its own `.tscn`.
...
Until the first overlay exists, the master arterial backbone + deck is the only cross-district
carrier.
