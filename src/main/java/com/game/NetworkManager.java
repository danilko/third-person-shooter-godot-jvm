...
import com.character.FirearmItem;
...
public class NetworkManager extends Node {
    ...
    public void sendBaselineVehicleOccupancy(int targetPeerId) {
        ...
        for (Node node : getTree().getNodesInGroup(CHARACTERS_GROUP)) {
            if (!(node instanceof com.vehicle.Vehicle v) || v.getCharacterInfo() == null) continue;
            Character occupant = v.getOccupant();
    ...
    private void broadcastVehicleSnapshots() {
        ...
        for (Node node : getTree().getNodesInGroup(CHARACTERS_GROUP)) {
            ...
            if (v.getController() instanceof com.vehicle.VehicleNetworkController vnc) {
    ...
    /** Splits the gathered vehicle entries into ≤{@link #MAX_BATCH_PAYLOAD_BYTES} frames — same pattern as broadcastSnapshotBatchChunked. */
    private void broadcastVehicleBatchChunked(List<NetMessageCodec.DecodedVehicleSnapshot> entries) {
        if (entries.isEmpty()) return;
        List<NetMessageCodec.DecodedVehicleSnapshot> chunk = new ArrayList<>();
        int chunkBytes = 3;   // tag u8 + count u16
        for (NetMessageCodec.DecodedVehicleSnapshot e : entries) {
            int entryBytes = NetMessageCodec.VEHICLE_SNAPSHOT_ENTRY_FIXED_BYTES + e.vehicleId().length();
            if (!chunk.isEmpty() && chunkBytes + entryBytes > MAX_BATCH_PAYLOAD_BYTES) {
                com.game.net.NetStats.increment("vehicle_batch_split");
                broadcastMessage(NetMessageCodec.encodeVehicleSnapshotBatch(MSG_VEHICLE_SNAPSHOT_BATCH, chunk), null);
                chunk = new ArrayList<>();
                chunkBytes = 3;
            }
            chunk.add(e);
    ...
    private void broadcastSnapshotBatchChunked(List<NetMessageCodec.DecodedSnapshot> entries) {
        if (entries.isEmpty()) return;
        List<NetMessageCodec.DecodedSnapshot> chunk = new ArrayList<>();
        int chunkBytes = 3;   // tag u8 + count u16
        for (NetMessageCodec.DecodedSnapshot e : entries) {
            int entryBytes = SNAPSHOT_ENTRY_FIXED_BYTES + e.characterId().length();
            if (!chunk.isEmpty() && chunkBytes + entryBytes > MAX_BATCH_PAYLOAD_BYTES) {
                com.game.net.NetStats.increment("snapshot_batch_split");
                broadcastMessage(NetMessageCodec.encodeSnapshotBatch(MSG_SNAPSHOT_BATCH, chunk), null);
                chunk = new ArrayList<>();
                chunkBytes = 3;
            }
            chunk.add(e);
    ...
    private void handlePeerConnected(ENetPacketPeer peer) {
        ...
        if (amServer) {
            Node managerNode = getNodeOrNull("/root/GameManager");
            if (managerNode instanceof GameManager manager) manager.onPeerConnected(id);
        } else {
    ...
    private void removeLocalPrePlacedPlayer() {
        ...
        for (Node node : getTree().getNodesInGroup(CHARACTERS_GROUP)) {
            if (node instanceof Player player) {
                ...
                if (player.currentVehicleNode instanceof com.vehicle.Vehicle vehicle) {
                    vehicle.tryExit();
                }
    ...
    private void onPacketReceived(int senderPeerId, PackedByteArray data) {
        try {
            ...
            if (bucket != null && !bucket.tryConsume()) {
                com.game.net.NetStats.increment("drop_rate_limited");
            ...
            com.game.net.NetStats.increment("drop_malformed");
    ...
    private void applySnapshotEntry(int senderPeerId, NetMessageCodec.DecodedSnapshot snap) {
        if (!isValidSnapshot(snap)) {
            com.game.net.NetStats.increment("drop_invalid_snapshot");
            GD.print("NetworkManager: dropping invalid MSG_SNAPSHOT entry from " + senderPeerId);
            return;
        }
        ...
        if (character == null) {
            ...
            com.game.net.NetStats.increment("snapshot_unknown_character");
    ...
    private void applyVehicleSnapshotEntry(int senderPeerId, NetMessageCodec.DecodedVehicleSnapshot snap) {
        if (!isValidVehicleSnapshot(snap)) {
            com.game.net.NetStats.increment("drop_invalid_vehicle_snapshot");
            GD.print("NetworkManager: dropping invalid vehicle snapshot from " + senderPeerId);
            return;
        }
        if (!(findControllableById(snap.vehicleId()) instanceof com.vehicle.Vehicle vehicle)) {
            com.game.net.NetStats.increment("vehicle_snapshot_unknown");
        ...
        if (isServer()) {
            ...
            if (vehicle.getCharacterInfo().ownerPeerId != senderPeerId) {
                com.game.net.NetStats.increment("vehicle_snapshot_not_owner");
                return;
            }
        ...
        if (vehicle.getController() instanceof com.vehicle.VehicleNetworkController vnc) {
     ...
     */
    public void sendOwnedVehicleState(com.vehicle.Vehicle vehicle) {
    ...
    private void handleVehicleSeatRequestMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidIdentifier(req.vehicleId()) || !isValidIdentifier(req.characterId())) {
            GD.print("NetworkManager: dropping invalid MSG_VEHICLE_SEAT_REQUEST from " + senderPeerId);
            return;
        }
        if (!isServer()) return;
        Node managerNode = getNodeOrNull("/root/GameManager");
        if (managerNode instanceof GameManager manager) {
    ...
    private void handleVehicleOccupancyMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidIdentifier(occ.vehicleId()) || !isBoundedString(occ.occupantCharacterId())
                || occ.ownerPeerId() < 0) {
            GD.print("NetworkManager: dropping invalid MSG_VEHICLE_OCCUPANCY from " + senderPeerId);
            return;
        }
        if (isServer()) return;
        Node managerNode = getNodeOrNull("/root/GameManager");
    ...
    private void handleIdentifyMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidIdentify(id)) {
            GD.print("NetworkManager: dropping invalid MSG_IDENTIFY from " + senderPeerId);
            return;
        }
        if (isServer()) {
    ...
    private void handleDamageRequestMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidDamageRequest(req)) {
            GD.print("NetworkManager: dropping invalid MSG_DAMAGE_REQUEST from " + senderPeerId);
            return;
        }
        // Entity-generic (N3): victims may be Characters or Vehicles — both carry Health.
        com.character.Controllable victim = findControllableById(req.victimCharacterId());
    ...
    private void handleDamageBroadcastMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidDamageBroadcast(cast)) {
            GD.print("NetworkManager: dropping invalid MSG_DAMAGE_BROADCAST from " + senderPeerId);
            return;
        }
        com.character.Controllable victim = findControllableById(cast.victimCharacterId());
    ...
    private void handleShotMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidShot(shot)) {
            com.game.net.NetStats.increment("shot_invalid");
            GD.print("NetworkManager: dropping invalid MSG_SHOT from " + senderPeerId);
            return;
        }
        if (!isServer()) return;
        ...
        if (shooter == null || shooter.characterInfo == null) {
            ...
            com.game.net.NetStats.increment("shot_unknown_shooter");
        ...
        if (shooter.characterInfo.ownerPeerId != senderPeerId) {
            com.game.net.NetStats.increment("shot_not_owner");
            GD.print("NetworkManager: dropping MSG_SHOT from peer " + senderPeerId
        ...
        if (wc == null) {
            com.game.net.NetStats.increment("shot_no_weapon_controller");
        ...
        if (firearm == null) {
            ...
            com.game.net.NetStats.increment("shot_slot_mismatch");
            ...
            if (firearm != null) {
                com.game.net.NetStats.increment("shot_slot_fallback");
                ...
                com.game.net.NetStats.increment("shot_dropped_no_firearm");
                GD.print("NetworkManager: dropping MSG_SHOT for " + shot.shooterCharacterId()
    ...
    private void handleWorldEventMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidWorldEvent(event)) {
            GD.print("NetworkManager: dropping invalid MSG_WORLD_EVENT from " + senderPeerId);
            return;
        }
    ...
    private void handleOwnershipMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidIdentifier(own.characterId()) || own.newOwnerPeerId() < 0) {
            GD.print("NetworkManager: dropping invalid MSG_OWNERSHIP from " + senderPeerId);
            return;
        }
        ...
        if (isServer()) {
            com.game.net.NetStats.increment("ownership_client_rejected");
    ...
    private CharacterInfo findCharacterInfoById(String characterId) {
        com.character.Controllable c = findControllableById(characterId);
    ...
    private void handleSpawnMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidSpawn(spawn)) {
            GD.print("NetworkManager: dropping invalid MSG_SPAWN from " + senderPeerId);
            return;
        }
        if (isServer()) return;

        Node managerNode = getNodeOrNull("/root/GameManager");
        if (managerNode instanceof GameManager manager) manager.spawnReplicatedCharacter(spawn);
    }
    ...
    private void handlePickupRequestMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidIdentifier(req.pickupId()) || !isValidIdentifier(req.characterId())) {
            GD.print("NetworkManager: dropping invalid MSG_PICKUP_REQUEST from " + senderPeerId);
            return;
        }
        if (!isServer()) return;
        Node managerNode = getNodeOrNull("/root/GameManager");
    ...
    private void handlePickupTakenMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidIdentifier(taken.pickupId()) || !isValidIdentifier(taken.characterId())
            ...
            GD.print("NetworkManager: dropping invalid MSG_PICKUP_TAKEN from " + senderPeerId);
            return;
        }
        if (isServer()) return;
        Node managerNode = getNodeOrNull("/root/GameManager");
    ...
    public void sendBaselinePickups(int targetPeerId) {
        ...
        for (Node node : getTree().getNodesInGroup(new StringName(com.environment.Pickup.PICKUPS_GROUP))) {
            if (!(node instanceof com.character.WeaponItem item) || !item.isTaken()) continue;
    ...
    private void handleWeaponDroppedMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidWeaponDropped(drop)) {
            GD.print("NetworkManager: dropping invalid MSG_WEAPON_DROPPED from " + senderPeerId);
            return;
        }
        if (isServer()) {
    ...
    private void handleEliminationMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidElimination(elim)) {
            com.game.net.NetStats.increment("drop_invalid_elimination");
            GD.print("NetworkManager: dropping invalid MSG_ELIMINATION from " + senderPeerId);
            return;
        }
        if (isServer()) return;
        Node managerNode = getNodeOrNull("/root/GameManager");
        if (managerNode instanceof GameManager manager) manager.applyReplicatedElimination(elim);
    }
    ...
    private void handleInventoryMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidInventory(inv)) {
            com.game.net.NetStats.increment("drop_invalid_inventory");
            GD.print("NetworkManager: dropping invalid MSG_INVENTORY from " + senderPeerId);
            return;
        }
        if (isServer()) return;
        Node managerNode = getNodeOrNull("/root/GameManager");
        if (managerNode instanceof GameManager manager) manager.applyReplicatedInventory(inv);
    }
    ...
    private void handleDespawnMessage(int senderPeerId, StreamPeerBuffer buf) {
        ...
        if (!isValidDespawn(despawn)) {
            GD.print("NetworkManager: dropping invalid MSG_DESPAWN from " + senderPeerId);
            return;
        }
        if (isServer()) return;

        Node managerNode = getNodeOrNull("/root/GameManager");
        if (managerNode instanceof GameManager manager) manager.despawnReplicatedCharacter(despawn.characterId());
    }
     ...
     */
    private com.character.Controllable findControllableById(String characterId) {
