...
public class AimDebugHost extends Node3D {
	...
	/**
	 * Every body that can be put on the stand: the {@code CharacterVisuals_*.tscn} scenes beside the
	 * reference. DISCOVERED rather than listed, so a body added by
	 * {@code tools/godot/build_character_visuals.gd} appears here with no edit -- which is the whole
	 * point, since the reason to look is to compare a new body against the reference.
	 */
	private static final String VISUALS_DIR = "res://src/main/resources/com/openworld/character/";
	private static String[] benchBodies() {
		java.util.List<String> out = new java.util.ArrayList<>();
		DirAccess dir = DirAccess.Companion.open(VISUALS_DIR);
		if (dir != null) {
			dir.listDirBegin();
			for (String f = dir.getNext(); f != null && !f.isEmpty(); f = dir.getNext()) {
				// an imported scene is listed as <name>.tscn.remap in an exported build
				String name = f.endsWith(".remap") ? f.substring(0, f.length() - 6) : f;
				if (name.startsWith("CharacterVisuals_") && name.endsWith(".tscn")) {
					out.add(VISUALS_DIR + name);
				}
			}
			dir.listDirEnd();
		}
	...
	private static final String[] BENCH_BODIES = benchBodies();
	/** Which of {@link #BENCH_BODIES} the mannequin is wearing; -1 = whatever the scene authors. */
	private int benchBodyIndex = -1;
	...
	private void spawnMannequin(Vector3 at) {
		...
		mannequin = ai;
		// The body is chosen BEFORE add_child, because `Character._ready` is what instances the
		// visuals and wires every path out of their MeshConfig -- set it after and the body is the
		// scene's own with a second one hanging off it.
		if (benchBodyIndex >= 0 && benchBodyIndex < BENCH_BODIES.length && ai instanceof Character cv) {
			if (GD.INSTANCE.load(BENCH_BODIES[benchBodyIndex]) instanceof PackedScene vis) {
				cv.characterVisuals = vis;
			}
		}
	...
	public void _input(InputEvent event) {
		...
		else if (code == Key.V) cycleBenchBody(k.isShiftPressed() ? -1 : 1);
		else if (code == Key.M) { if (k.isShiftPressed()) startReview(); else sweepBenchWeapons(); }
	}
	 ...
	 */
	/**
	 * Put the next body on the stand ({@code V}). A body is a fact about proportions -- reach,
	 * shoulder width, where the eye is -- so the only way to judge a shared pose is to see it worn
	 * by each of them, and the readout line below says which one is on.
	 */
	private void cycleBenchBody(int dir) {
		if (BENCH_BODIES.length == 0) {
			GD.INSTANCE.print("[AimBench] no CharacterVisuals_*.tscn found in " + VISUALS_DIR);
			return;
		}
		benchBodyIndex = Math.floorMod(benchBodyIndex + 1 + dir, BENCH_BODIES.length + 1) - 1;
		GD.INSTANCE.print("[AimBench] body -> " + benchBodyName());
		respawnMannequin();
	}

	/** The body on the stand, for the overlay and the log. */
	private String benchBodyName() {
		if (benchBodyIndex < 0 || benchBodyIndex >= BENCH_BODIES.length) return "(the scene's own)";
		String p = BENCH_BODIES[benchBodyIndex];
		return p.substring(p.lastIndexOf('/') + 1).replace("CharacterVisuals_", "").replace(".tscn", "");
	}

	/**
	 * Every bench weapon on the CURRENT body, one measured line each ({@code M}). It is the same
	 * three numbers the fit gate asserts -- stock, support grip and gun-off-aim -- but here they
	 * can be watched rather than read out of a log, which is the thing a shared pose needs: a gate
	 * says PASS, a picture says whether the hold looks like a hold.
	 */
	private void sweepBenchWeapons() {
		GD.INSTANCE.print("[AimBench] sweep on " + benchBodyName() + " / " + mannequinStance + ":");
		sweepRemaining = BENCH_WEAPONS.length;
		sweepWait = 0;                 // arm the first one on the next frame
		sweepStarted = false;
	}

	// ── The review: every BODY x every WEAPON x every on-foot STANCE, in one run ──────────────
	//
	// The gap this closes is a real one and it is not a missing gate. `probe_weapon_fit` measures
	// weapon x stance for ONE body per run; `V` + `M` measure body x weapon at ONE stance; the
	// Blender side measures the CLIP, before any modifier. So "how does the throw pose look on
	// Shino crawling" took stitching two tools together, and nothing showed the matrix at once.
	// This walks it on the shipped path and logs one line per cell -- and it deliberately does NOT
	// assert anything: the gates answer "did this regress", this answers "show me".
	private static final StanceName[] REVIEW_STANCES = {StanceName.UPRIGHT, StanceName.CROUCH, StanceName.CRAWL};
	private final java.util.ArrayDeque<int[]> reviewCells = new java.util.ArrayDeque<>();
	private boolean reviewActive = false;

	private void startReview() {
		reviewCells.clear();
		// the scene's own body first (index -1), then every discovered one
		for (int b = -1; b < BENCH_BODIES.length; b++) {
			for (int st = 0; st < REVIEW_STANCES.length; st++) reviewCells.add(new int[] {b, st});
		}
		reviewActive = true;
		// PUT THE TARGET WHERE THE BODY IS FACING. The stand spawns the mannequin facing away from
		// the aim ball, so every number taken before the ball is moved round is a reading of the
		// stance's YAW CLAMP, not of the pose -- measured, the same body and weapon read grip
		// 0.037 m / shoulder 38.8 deg aiming backwards and 0.002 m / 0.0 deg aiming forwards. A
		// review whose headline number means nothing is worse than no review.
		targetPlayer = false;
		// Combat ON explicitly, never by pressing the toggle: `4` flips it, the mannequin spawns
		// with it already on, and a review that "turns combat on" that way turns it OFF -- which is
		// exactly what happened, and it is invisible in the numbers unless the line says so (every
		// cell read a plausible stock and support while the gun sat 55-123 deg off the target,
		// because with combat off the aim branch never runs).
		if (pose != null) pose.wantCombat = true;
		faceTheBall();
		GD.INSTANCE.print(String.format("[AimBench] REVIEW: %d bodies x %d weapons x %d stances = %d cells",
				BENCH_BODIES.length + 1, BENCH_WEAPONS.length, REVIEW_STANCES.length,
				reviewCells.size() * BENCH_WEAPONS.length));
		nextReviewCell();
	}

	/**
	 * Turn the mannequin to face the aim ball, and keep it turned for the whole review.
	 *
	 * <p>The stand parks the mannequin facing away from the ball on purpose (it is a shooting bench
	 * as well as a pose bench), so every fit number read before it is turned is a reading of the
	 * STANCE'S YAW CLAMP with the arms wrapped round the body -- measured, the same body and weapon
	 * read grip 0.037 m / shoulder 38.8 deg facing away and 0.000 / 0.0 facing the target. Turning
	 * the BODY rather than moving the ball is what survives the per-cell respawn, which restores the
	 * body's yaw and would otherwise put it back.
	 */
	private void faceTheBall() {
		if (mannequin == null || aimBall == null) return;
		Vector3 d = aimBall.getGlobalPosition().minus(mannequin.getGlobalPosition());
		if (d.lengthSquared() < 1e-4) return;
		// -Z is forward, so this is MovementController.aimYaw's own atan2(-dx, -dz).
		bodyYawDegrees = Math.toDegrees(Math.atan2(-d.getX(), -d.getZ()));
		mannequin.setRotation(new Vector3(0.0, Math.toRadians(bodyYawDegrees), 0.0));
	}

	private void nextReviewCell() {
		if (reviewCells.isEmpty()) {
			reviewActive = false;
			GD.INSTANCE.print("[AimBench] REVIEW done.");
			return;
		}
		int[] cell = reviewCells.poll();
		if (cell[0] != benchBodyIndex) {
			benchBodyIndex = cell[0];
			respawnMannequin();
		}
		setMannequinStance(REVIEW_STANCES[cell[1]]);
		sweepBenchWeapons();
	}

	/**
	 * One weapon per {@link #SWEEP_SETTLE_FRAMES}, driven from the frame step rather than run in a
	 * loop inside the key handler. Arming frees and rebuilds the mannequin, and the numbers being
	 * read come from the skeleton MODIFIERS -- which have not run yet on the frame the body is
	 * built, so a loop reports "no mount, no SupportPoint, NaN" for every weapon. Measured: that is
	 * exactly what the first version printed.
	 */
	// Long enough for the whole shipped equip to land: the respawn, the DEFERRED requestEquip, the
	// slot sync, the ~0.45 s weapon transition and the draw settle. At 40 frames every line read
	// "no mount, no SupportPoint" -- the body was still holding the FIST, which has neither.
	private static final int SWEEP_SETTLE_FRAMES = 110;
	private int sweepRemaining = 0;
	private int sweepWait = 0;

	private void stepBenchSweep() {
		if (sweepRemaining <= 0) return;
		if (sweepWait > 0) {
			sweepWait--;
			return;
		}
		if (sweepRemaining < BENCH_WEAPONS.length || sweepStarted) {
			GD.INSTANCE.print(String.format("[AimBench]   %-10s %-8s %-5s %s", benchBodyName(),
					mannequinStance, BENCH_WEAPONS[benchWeaponIndex], fitLine()));
			if (--sweepRemaining <= 0) {
				sweepStarted = false;
				if (reviewActive) nextReviewCell();
				return;
			}
		}
		sweepStarted = true;
		cycleBenchWeapon(1);
		sweepWait = SWEEP_SETTLE_FRAMES;
	}
	...
	/** Free the mannequin and build it again, keeping where it stood, its yaw, combat and trigger. */
	private void respawnMannequin() {
	...
	private String workbenchHeader() {
		...
		b.append(String.format("bench: body %s [V]   weapon %s [ /] [M sweeps all]   target %s   cover %s   player %s (hp %s)\n",
				benchBodyName(), BENCH_WEAPONS[benchWeaponIndex], targetPlayer ? "PLAYER" : "ball",
				coverWall != null ? "ON" : "off", playerInvulnerable ? "INVULNERABLE" : "mortal",
				healthText(player)));
		// The three numbers that say whether this SHARED pose fits THIS body with THIS weapon.
		b.append("fit  : ").append(fitLine()).append("\n");
	 ...
	 */
	/**
	 * The three numbers that decide whether a shared aim pose FITS the body wearing it, read off the
	 * live modifiers rather than recomputed: how far the stock mount had to drag the firing hand and
	 * whether it still fell short, how far the support hand is from its grip and how much shoulder
	 * that cost, and how far the gun ends up from what the body is aiming at.
	 *
	 * <p>They are the same quantities {@code tools/godot/probe_weapon_fit.gd} asserts. The gate says
	 * PASS; this says whether the hold looks like a hold, on this body, with this weapon, while you
	 * watch it.
	 */
	private String fitLine() {
		Skeleton3D skel = findSkeleton(mannequin);
		if (skel == null) return "(no skeleton)";
		String stock = "no mount";
		if (skel.getNodeOrNull(new NodePath("StockMountIKModifier"))
				instanceof com.openworld.character.StockMountIKModifier sm && !sm.currentMount().isEmpty()) {
			stock = String.format("%s moved %.3f m, short %.3f m", sm.currentMount(),
					sm.lastHandMove(), sm.lastShortfall());
		}
		String support = "no SupportPoint";
		if (skel.getNodeOrNull(new NodePath("SupportHandIKModifier"))
				instanceof com.openworld.character.SupportHandIKModifier sh && sh.lastGripMiss() >= 0.0) {
			support = String.format("grip %.3f m off (reach %.2f / arm %.2f, shoulder %.1f deg)",
					sh.lastGripMiss(), sh.lastTargetReach(), sh.armReach(), sh.lastClavicleDeg());
		}
		// The stance the BODY is in, not the one that was asked for: a cell that silently failed to
		// change stance reports the same numbers three times, which is indistinguishable from a
		// pose that does not vary by stance.
		StanceName live = (mannequin instanceof Character sc) ? stanceForOrdinal(sc.getStanceOrdinal()) : null;
		return String.format("[%s combat=%s] holding %s | stock: %s | support: %s | gun %.1f deg off aim",
				live, (mannequin instanceof Character cc) && cc.isCombat(), heldName(mannequin), stock, support,
				gunAimErrorOn(mannequin, aimBall));
	}
	...
	private void buildCases() {
		...
		// Crawl's upward band is TIGHT and deliberately asymmetric (W45): the authored prone pose
		// already spends 66.9 deg of neck to look level forward -- the largest neck rotation in the
		// library -- so there is very little left above it, and the symmetric +-30 it used to carry
		// asked for 30 deg more on top of that. Shooting upward is what standing up is for.
		cases.add(Case.limit("limit up, crawl", "crawl", -12000.0, 15.0).gun(-999, 10.0));
