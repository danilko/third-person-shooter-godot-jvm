#!/usr/bin/env bash
# Symlinks assets/world_source/addons/road_kit_authoring into every Blender version
...
#
# Usage: tools/install_blender_addon.sh   (run from anywhere; paths are absolute)

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ADDON_SRC="$REPO_ROOT/assets/world_source/addons/road_kit_authoring"
...
for version_dir in "$CONFIG_ROOT"/*/; do
    ...
    if [[ -L "$link_path" ]]; then
        ...
        if [[ "$current_target" == "$ADDON_SRC" ]]; then
